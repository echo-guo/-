# -*- coding: utf-8 -*-
"""
Created on Wed Jun 27 16:52:25 2018
...
for i in ls:
    ...
    if d[0]=='{':
        ...
        f.close()
#######################################循环全部（老师）
f=open('C:/Users/Administrator/Desktop/h.txt','w')
import urllib.request as r
for j in range(2300):
    url='http://www.gaokaopai.com/university-ajaxGetMajor.html'
    data='id={}&type=2&city=46&state=1'.format(ls[j]).encode()
    req=r.Request(url,data=data,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.146 Safari/537.36','X-Requested-With':'XMLHttpRequest'})
    d=r.urlopen(req).read().decode('utf-8','ignore')
    if d.startswith('{'):
        print('successfully')
        ...
        while True:
            print('{}下载失败'.format(j))
            url='http://www.gaokaopai.com/university-ajaxGetMajor.html'
            data='id={}&type=2&city=46&state=1'.format(j).encode()
            req=r.Request(url,data=data,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.146 Safari/537.36','X-Requested-With':'XMLHttpRequest'})
            d=r.urlopen(req).read().decode('utf-8','ignore')
            if d.startswith('{'):
                break
...
import urllib.request as r
url='http://www.gaokaopai.com/university-ajaxGetMajor.html'
data='id=477&type=2&city=46&state=1'.encode()
req=r.Request(url,data=data,headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.146 Safari/537.36','X-Requested-With':'XMLHttpRequest'})
d=r.urlopen(req).read().decode('utf-8','ignore')
import json
d=json.loads(d)
